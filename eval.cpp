...
Eval::Eval() {
    ...
    readTable("patterns/edgetable.txt", 729, edgeTable);
    readTable("patterns/p24table.txt", 729, p24Table);
...
int Eval::heuristic(Board &b, int turn, int s) {
    ...
    int patterns = 2*boardTo24PV(b, turn) + boardToEPV(b, turn)
            + 4*boardToE2XPV(b, turn) + 2*boardTo33PV(b, turn)
...
int Eval::heuristic2(Board &b, int turn, int s) {
    ...
    bitbrd bm = b.getBits(s);
    bitbrd bo = b.getBits(s^1);
    score += 4000 * (countSetBits(bm&CORNERS) - countSetBits(bo&CORNERS));
    score += 100 * (countSetBits(bm&EDGES) - countSetBits(bo&EDGES));
    score -= 800 * (countSetBits(bm&X_CORNERS) - countSetBits(bo&X_CORNERS));
    score -= 200 * (countSetBits(bm&ADJ_CORNERS) - countSetBits(bo&ADJ_CORNERS));
